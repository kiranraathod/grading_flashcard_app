...
class FlashcardBloc extends Bloc<FlashcardEvent, FlashcardState> {
  ...
  void _setupRepositoryListeners() {
    ...
    _setsSubscription = _repository.watchAll().listen(
      (sets) {
        debugPrint('🔄 FlashcardBloc: Repository data changed - ${sets.length} sets');
    ...
    _syncSubscription = _repository.syncStatus.listen(
      (syncStatus) {
        debugPrint('🔄 FlashcardBloc: Sync status changed - $syncStatus');
        
        if (state is FlashcardLoaded) {
          add(FlashcardSyncStatusUpdated(
            syncStatus: syncStatus,
            isSyncing: syncStatus == SyncStatus.syncing,
            lastSyncTime: syncStatus == SyncStatus.synced 
                ? DateTime.now() 
                : null,
          ));
        }
  ...
  Future<void> _onLoadRequested(
    ...
    try {
      ...
      emit(FlashcardLoaded(
        sets: sets,
        syncStatus: _repository.isSyncing ? SyncStatus.syncing : SyncStatus.idle,
  ...
  Future<void> _onSetAdded(
    ...
    debugPrint('➕ FlashcardBloc: Adding flashcard set - ${event.flashcardSet.title}');
    ...
    try {
      ...
      emit(FlashcardError(
        ...
        operation: 'add',
        error: error,
        cachedSets: state is FlashcardLoaded ? (state as FlashcardLoaded).sets : null,
      ));
    }
  }  
  ...
  Future<void> _onSetUpdated(
    ...
    debugPrint('✏️ FlashcardBloc: Updating flashcard set - ${event.flashcardSet.id}');
  ...
  Future<void> _onProgressUpdated(
    ...
    debugPrint('📊 FlashcardBloc: Updating progress - Set: ${event.setId}, Card: ${event.cardId}, Completed: ${event.isCompleted}');
  ...
  Future<void> _onMarkedForReview(
    ...
    debugPrint('🔖 FlashcardBloc: Marking for review - Set: ${event.setId}, Card: ${event.cardId}, Marked: ${event.isMarkedForReview}');
    ...
    try {
      ...
      final updatedFlashcards = currentSet.flashcards.map((card) {
  ...
  Future<void> _onSearchRequested(
    ...
    try {
      ...
      debugPrint('✅ FlashcardBloc: Search complete - ${filteredSets.length} results');
  ...
  Future<void> _onSyncRequested(
    ...
    debugPrint('🔄 FlashcardBloc: Sync requested - forceRefresh: ${event.forceRefresh}');
