...
class FlashcardRepository extends BaseRepositoryImpl<FlashcardSet> 
  ...
  void _onConnectivityChanged() {
    ...
    if (_connectivityService.isOnline && _currentSyncStatus != SyncStatus.syncing) {
  ...
  Future<void> _loadFromCache() async {
    ...
    if (setsData != null && setsData.isNotEmpty) {
      _cachedSets = setsData.map((data) => FlashcardSet.fromJson(data)).toList();
  ...
  Future<void> resolveSyncConflicts() async {
    ...
    logOperation('resolveSyncConflicts', metadata: {'strategy': 'lastModifiedWins'});
  }
  ...
  void validateItem(FlashcardSet item) {
    ...
    final existingSets = _cachedSets.where((set) => 
      set.id != item.id && 
  ...
  Future<void> updateCardProgress({
    ...
    return safeOperation('updateCardProgress', () async {
      ...
      final updatedFlashcards = set.flashcards.map((card) {
        if (card.id == cardId) {
          return Flashcard(
            id: card.id,
      ...
      logOperation('updateCardProgress', metadata: {
        'setId': setId,
        'cardId': cardId, 
        'isCompleted': isCompleted,
      });
  ...
  Future<List<FlashcardSet>> getSetsByCompletionStatus(bool completed) async {
    return safeOperation('getSetsByCompletionStatus', () async {
      ...
      return allSets.where((set) {
        final completedCards = set.flashcards.where((card) => card.isCompleted).length;
        final isSetCompleted = completedCards == set.flashcards.length && set.flashcards.isNotEmpty;
