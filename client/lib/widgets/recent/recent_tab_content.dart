...
class _RecentTabContentState extends State<RecentTabContent> with AutomaticKeepAliveClientMixin {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocConsumer<RecentViewBloc, RecentViewState>(
      listener: (context, state) {
        if (state is RecentViewError) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Error loading recent items: ${state.message}'),
      ...
      builder: (context, state) {
        debugPrint('⭐ Building RecentTabContent with state: ${state.runtimeType}');
  ...
  Widget _buildContent(RecentViewLoaded state) {
    ...
    return Column(
      ...
      mainAxisSize: MainAxisSize.min, // Important: Use min to avoid the unbounded height issue
      children: [
        // Stats summary
        ...
        SizedBox(
          height: listHeight > 300 ? listHeight : 300, // Use calculated height with a minimum
  ...
  Widget _buildStatsSummary(List<RecentlyViewedItem> items) {
    final flashcardCount = items.where((item) => item.type == RecentItemType.flashcard).length;
    final interviewCount = items.where((item) => item.type == RecentItemType.interviewQuestion).length;
    
    String lastStudiedText = 'Never';
    if (items.isNotEmpty) {
      final mostRecent = items.reduce((a, b) => a.viewedAt.isAfter(b.viewedAt) ? a : b);
      final now = DateTime.now();
      final difference = now.difference(mostRecent.viewedAt);
      
      if (difference.inMinutes < 60) {
        lastStudiedText = '${difference.inMinutes} minute${difference.inMinutes == 1 ? '' : 's'} ago';
      } else if (difference.inHours < 24) {
        lastStudiedText = '${difference.inHours} hour${difference.inHours == 1 ? '' : 's'} ago';
      } else {
        lastStudiedText = '${difference.inDays} day${difference.inDays == 1 ? '' : 's'} ago';
      }
    }
  ...
  Widget _buildEmptyFilteredState() {
    final bool isFlashcardFilter = _filterType != null && _filterType == RecentItemType.flashcard;
    final filterName = isFlashcardFilter
        ? 'flashcards'
        : 'interview questions';
      ...
      child: Column(
        ...
        children: [
          ...
          ElevatedButton.icon(
            ...
            style: ElevatedButton.styleFrom(
              backgroundColor: isFlashcardFilter
                  ? AppColors.primary
                  : Colors.purple,
              foregroundColor: Colors.white,
            ),
