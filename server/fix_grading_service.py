      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          if (_isGenericMockResponse(responseData, answer.userAnswer)) {
            debugPrint('⚠️ Detected generic mock response, using client-side grading instead.');
...
'''
    ...
    try:
        with open(api_service_path, 'w') as f:
            f.write(fix_content)
        logger.info(f"✅ Successfully created the fixed API service at {api_service_path}")
        return True
    except Exception as e:
        logger.error(f"❌ Failed to write the fixed API service: {str(e)}")
        return False
